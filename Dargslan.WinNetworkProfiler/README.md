# Dargslan.WinNetworkProfiler — 2026 Edition

> **Windows network profiling toolkit — network adapter deep analysis, WiFi profile audit, NIC teaming status, VLAN configuration, and bandwidth testing (2026 Edition)**

[![PowerShell Gallery](https://img.shields.io/badge/PowerShell%20Gallery-Dargslan.WinNetworkProfiler-blue)](https://www.powershellgallery.com/packages/Dargslan.WinNetworkProfiler)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![PowerShell 5.1+](https://img.shields.io/badge/PowerShell-5.1+-blue.svg)](https://docs.microsoft.com/en-us/powershell/)
[![2026 Edition](https://img.shields.io/badge/Edition-2026-green.svg)](https://dargslan.com)

Part of the [Dargslan Windows Admin Toolkit](https://dargslan.com) — professional tools for Windows system administrators.

## Installation

```powershell
Install-Module -Name Dargslan.WinNetworkProfiler -Scope CurrentUser
```

## Quick Start

```powershell
Import-Module Dargslan.WinNetworkProfiler
Get-NetworkProfile
Get-NetworkProfile -Json
```

## Functions

### `Get-NetworkProfile`

Generates detailed network profiles including adapter capabilities, WiFi configurations, NIC teams, and connectivity analysis.

```powershell
Get-NetworkProfile
Get-NetworkProfile -Json
```



## Use Cases

- **Network Inventory**: Deep adapter and configuration profiling
- **WiFi Security**: Audit stored WiFi profiles
- **NIC Teaming**: Monitor team status and configuration
- **VLAN Management**: Verify VLAN assignments
- **Troubleshooting**: Comprehensive network diagnostics

## Requirements

- **PowerShell 5.1+** or **PowerShell 7+**
- **Windows 10/11** or **Windows Server 2016+**
- Some functions require **Administrator** privileges

## More Dargslan Tools

| Resource | Link |
|----------|------|
| Homepage | [dargslan.com](https://dargslan.com) |
| Free Cheat Sheets | [dargslan.com/cheat-sheets](https://dargslan.com/cheat-sheets) |
| eBooks | [dargslan.com/books](https://dargslan.com/books) |
| PowerShell Gallery | [powershellgallery.com/profiles/Dargslan](https://www.powershellgallery.com/profiles/Dargslan) |

## License

MIT License — (c) 2026 Dargslan — [dargslan.com](https://dargslan.com)
