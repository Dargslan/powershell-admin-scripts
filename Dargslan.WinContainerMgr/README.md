# Dargslan.WinContainerMgr — 2026 Edition

> **Windows container and Docker Desktop management toolkit — container inventory, image audit, network inspection, volume management, and resource monitoring (2026 Edition)**

[![PowerShell Gallery](https://img.shields.io/badge/PowerShell%20Gallery-Dargslan.WinContainerMgr-blue)](https://www.powershellgallery.com/packages/Dargslan.WinContainerMgr)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![PowerShell 5.1+](https://img.shields.io/badge/PowerShell-5.1+-blue.svg)](https://docs.microsoft.com/en-us/powershell/)
[![2026 Edition](https://img.shields.io/badge/Edition-2026-green.svg)](https://dargslan.com)

Part of the [Dargslan Windows Admin Toolkit](https://dargslan.com) — professional tools for Windows system administrators.

## Installation

```powershell
Install-Module -Name Dargslan.WinContainerMgr -Scope CurrentUser
```

## Quick Start

```powershell
Import-Module Dargslan.WinContainerMgr
Get-ContainerReport
Get-ContainerReport -Json
```

## Functions

### `Get-ContainerReport`

Reports Docker/container status, running containers, images, volumes, networks, and resource usage.

```powershell
Get-ContainerReport
Get-ContainerReport -Json
```

### `Invoke-ContainerCleanup`

Removes stopped containers, dangling images, unused volumes and networks to reclaim disk space.

```powershell
Invoke-ContainerCleanup
Invoke-ContainerCleanup -Json
```



## Use Cases

- **DevOps Monitoring**: Track container health and resource usage
- **Disk Management**: Clean up unused images and volumes
- **Security**: Audit container configurations and exposed ports
- **Development**: Monitor Docker Desktop resource consumption
- **CI/CD**: Verify container environments in build pipelines

## Requirements

- **PowerShell 5.1+** or **PowerShell 7+**
- **Windows 10/11** or **Windows Server 2016+**
- Some functions require **Administrator** privileges

## More Dargslan Tools

| Resource | Link |
|----------|------|
| Homepage | [dargslan.com](https://dargslan.com) |
| Free Cheat Sheets | [dargslan.com/cheat-sheets](https://dargslan.com/cheat-sheets) |
| eBooks | [dargslan.com/books](https://dargslan.com/books) |
| PowerShell Gallery | [powershellgallery.com/profiles/Dargslan](https://www.powershellgallery.com/profiles/Dargslan) |

## License

MIT License — (c) 2026 Dargslan — [dargslan.com](https://dargslan.com)
