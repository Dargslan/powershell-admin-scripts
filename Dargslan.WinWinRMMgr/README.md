# Dargslan.WinWinRMMgr — 2026 Edition

> **Windows Remote Management (WinRM) audit toolkit — WinRM configuration, trusted hosts, listener management, and security assessment (2026 Edition)**

[![PowerShell Gallery](https://img.shields.io/badge/PowerShell%20Gallery-Dargslan.WinWinRMMgr-blue)](https://www.powershellgallery.com/packages/Dargslan.WinWinRMMgr)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![PowerShell 5.1+](https://img.shields.io/badge/PowerShell-5.1+-blue.svg)](https://docs.microsoft.com/en-us/powershell/)
[![2026 Edition](https://img.shields.io/badge/Edition-2026-green.svg)](https://dargslan.com)

Part of the [Dargslan Windows Admin Toolkit](https://dargslan.com) — professional tools for Windows system administrators.

## Installation

```powershell
Install-Module -Name Dargslan.WinWinRMMgr -Scope CurrentUser
```

## Quick Start

```powershell
Import-Module Dargslan.WinWinRMMgr
Get-WinRMAudit
Get-WinRMAudit -Json
```

## Functions

### `Get-WinRMAudit`

Reports WinRM service status, listeners, trusted hosts, authentication settings, and security posture.

```powershell
Get-WinRMAudit
Get-WinRMAudit -Json
```



## Use Cases

- **Remote Management Security**: Audit WinRM attack surface
- **PowerShell Remoting**: Verify remoting configuration
- **Compliance**: Check WinRM against security baselines
- **Troubleshooting**: Debug remote management connectivity
- **Hardening**: Identify and fix WinRM security weaknesses

## Requirements

- **PowerShell 5.1+** or **PowerShell 7+**
- **Windows 10/11** or **Windows Server 2016+**
- Some functions require **Administrator** privileges

## More Dargslan Tools

| Resource | Link |
|----------|------|
| Homepage | [dargslan.com](https://dargslan.com) |
| Free Cheat Sheets | [dargslan.com/cheat-sheets](https://dargslan.com/cheat-sheets) |
| eBooks | [dargslan.com/books](https://dargslan.com/books) |
| PowerShell Gallery | [powershellgallery.com/profiles/Dargslan](https://www.powershellgallery.com/profiles/Dargslan) |

## License

MIT License — (c) 2026 Dargslan — [dargslan.com](https://dargslan.com)
