# Dargslan.WinWSLMgr — 2026 Edition

> **Windows Subsystem for Linux management toolkit — WSL distribution inventory, resource monitoring, configuration audit, and version management (2026 Edition)**

[![PowerShell Gallery](https://img.shields.io/badge/PowerShell%20Gallery-Dargslan.WinWSLMgr-blue)](https://www.powershellgallery.com/packages/Dargslan.WinWSLMgr)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![PowerShell 5.1+](https://img.shields.io/badge/PowerShell-5.1+-blue.svg)](https://docs.microsoft.com/en-us/powershell/)
[![2026 Edition](https://img.shields.io/badge/Edition-2026-green.svg)](https://dargslan.com)

Part of the [Dargslan Windows Admin Toolkit](https://dargslan.com) — professional tools for Windows system administrators.

## Installation

```powershell
Install-Module -Name Dargslan.WinWSLMgr -Scope CurrentUser
```

## Quick Start

```powershell
Import-Module Dargslan.WinWSLMgr
Get-WSLReport
Get-WSLReport -Json
```

## Functions

### `Get-WSLReport`

Reports installed WSL distributions, version, resource allocation, running instances, and configuration.

```powershell
Get-WSLReport
Get-WSLReport -Json
```



## Use Cases

- **Developer Environment**: Manage WSL distributions and resources
- **Security**: Audit WSL installations on enterprise endpoints
- **Resource Management**: Monitor WSL memory and CPU usage
- **Policy**: Enforce WSL version and distribution standards
- **DevOps**: Verify WSL setup for development environments

## Requirements

- **PowerShell 5.1+** or **PowerShell 7+**
- **Windows 10/11** or **Windows Server 2016+**
- Some functions require **Administrator** privileges

## More Dargslan Tools

| Resource | Link |
|----------|------|
| Homepage | [dargslan.com](https://dargslan.com) |
| Free Cheat Sheets | [dargslan.com/cheat-sheets](https://dargslan.com/cheat-sheets) |
| eBooks | [dargslan.com/books](https://dargslan.com/books) |
| PowerShell Gallery | [powershellgallery.com/profiles/Dargslan](https://www.powershellgallery.com/profiles/Dargslan) |

## License

MIT License — (c) 2026 Dargslan — [dargslan.com](https://dargslan.com)
